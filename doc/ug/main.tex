...
\section{制御構造}
\verb|if|，\verb|while|，\verb|for|をサポートしています．
...
\section{データ型}
プリミティブ型は，型のビット幅分の変数になります．VHDLではsigned型，Verilog HDLではsigned reg型になります．また，定義したクラス型のインスタンス変数を使用できます．インスタンス変数はfinalとして宣言する必要があります．

...
public class HDLCombinationExpr implements HDLExpr{
	...
	HDLType t = null;
	if(t1.getKind().hasWidth() && t1.getKind().isPrimitive() && t2.getKind().hasWidth() && t2.getKind().isPrimitive()){
	...
	if(op.isInfix()){
	    ...
	    case DROPHEAD: {
		HDLPrimitiveType t = (HDLPrimitiveType)args[0].getResultExpr().getType();
		return String.format("%s(%d - %s - 1 downto 0)", args[0].getResultExpr().getVHDL(), t.getWidth(), ((HDLValue)args[1]).getValue());
	    }
	    case TAKE: {
		...
		if(shift >= ta.getWidth()){
		    ...
		    str = String.format("(%d-1 downto %d => %s) & %s(%d downto %d)",
					t0.getWidth(),
					(msb-lsb+1), // shift,
					padding,
			...
			str = String.format("(%d-1 downto %d => %s) & %s",
					    t0.getWidth(),
					    msb+1,
					    padding,
					    args[0].getResultExpr().getVHDL());
	...
	if(op.isInfix()){
		...
		return String.format("{%s,%s}",
				     getPaddingBitInVerilog(args[0].getResultExpr().getVerilogHDL(), t0.getWidth()-1, t1.getWidth()-t0.getWidth()),
